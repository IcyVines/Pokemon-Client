...
var BattlePokemon = (function() {
	...
	BattlePokemon.prototype.tryTrap = function() {
		if (this.runImmunity('trapped') || this.ability == 'shadowtag') {
			this.trapped = true;
			return true;
		}
		return false;
	...
	Battle.prototype.boost = function(boost, target, source, effect) {
		...
		for (var i in boost) {
			...
			if (boost[i] !== 0  && target.boostBy(currentBoost)) {
				// No Normal Stat Reduction - !(target.hasType('Normal') && target !== source)
				success = true;
				var msg = '-boost';
				if (boost[i] < 0) {
					msg = '-unboost';
				...
				switch (effect.id) {
				case ('underdog'): //fallthrough
				case ('mindbreak'): //fallthrough
				case ('intimidate'): //fallthrough
				case ('illuminate'):
