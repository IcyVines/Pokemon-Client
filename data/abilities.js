...
exports.BattleAbilities = {
	...
	"damp": {
		desc: "While this Pokemon is active, no Pokemon on the field can use Selfdestruct or Explosion. This Pokemon resists Fire type moves and gains Special Defense, Defense, and loses Speed when hit with a water type attack.",
		shortDesc: "This Pokemon resists Fire type moves and prevents explosions.",
		id: "damp",
	...
	"dethrone": {
		...
		onBasePower: function(basePower, attacker, defender, move) {
			...
			var atkDef = defender.getStat('atk', false, true);
	...
	"filter": {
		desc: "This Pokemon receives 35% reduced damage from Super Effective attacks.",
		shortDesc: "This Pokemon receives 65% damage from super effective attacks.",
	...
	"fullbloom": {
		inherit: true,
		desc: "This Pokemon cannot be paralyzed or put to sleep at all times, and sun or rain raises its defense or special defense by 50%, respectively.",
	...
	"galewings": {
		desc: "This Pokemon's Flying-type moves have their priority increased by 1, but their damage is decreased by 20%.",
		shortDesc: "Gives priority to Flying-type moves but lowers damage.",
	...
	"intenseflames": {
		inherit: true,
		desc: "This Pokemon is immune to Water, and when hit by a water type move, all hazards (except mist) are removed.",
	...
	"leafguard": {
		desc: "This Pokemon cannot be poisoned, paralyzed, or burned and takes half damage from fire type attacks in sun.",
		shortDesc: "This Pokemon cannot get certain statuses and resists fire in sun.",
		onUpdate: function(pokemon) {
		...
		onImmunity: function(type) {
			if (type === 'psn' || type === 'par' || type === 'brn') return false;
		},
		onModifyAtkPriority: 6,
		onSourceModifyAtk: function(atk, attacker, defender, move) {
	...
	"mindbreak": {
		...
		onStart: function(pokemon) {
			...
			for (var i=0; i<foeactive.length; i++) {
				...
				if (foeactive[i].volatiles['substitute']) {
					// does it give a message?
					this.add('-activate',foeactive[i],'Substitute','ability: Mind Break','[of] '+pokemon);
				} else {
					this.add('-ability',pokemon,'Mind Break','[of] '+foeactive[i]);
	...
	"rapidfire": {
		desc: "Multi-hit moves have priority but lose 15% accuracy.",
		...
		onModifyMove: function(move, user, target) {
			if (move && move.multihit){
				...
				move.accuracy = move.accuracy * 0.85;
			}
		},
		id: "rapidfire",
		name: "Rapid Fire",
	...
	"sandveil": {
		desc: "This Pokemon's Evasion is boosted by 1.11x if the weather is Sandstorm. This Pokemon is also immune to residual Sandstorm damage.",
		shortDesc: "If Sandstorm is active, this Pokemon's evasion is 1.11x; immunity to Sandstorm.",
		onImmunity: function(type, pokemon) {
			if (type === 'sandstorm') return false;
		},
	...
	"serenegrace": {
		...
		onModifyMove: function(move) {
			if (move.secondaries) {
				this.debug('doubling secondary chance');
				for (var i=0; i<move.secondaries.length; i++) {
					move.secondaries[i].chance *= 1.75;
				}
			}
		},
		id: "serenegrace",
	...
	"slowstart": {
		desc: "After this Pokemon switches into the battle, its Attack and Speed stats are halved for two turns. For example, if this Pokemon has an Attack stat of 400, its Attack will be 200 until the effects of Slow Start wear off.",
		shortDesc: "On switch-in, this Pokemon's Attack and Speed are halved for 2 turns.",
		onStart: function(pokemon) {
	...
	"sniper": {
		desc: "When this Pokemon lands a Critical Hit, the damage is increased to another 1.5x. 100% chance for an increased critical hit ratio.",
		shortDesc: "If this Pokemon strikes with a critical hit, the damage is increased by 100%. 50% chance for increased crit. ratio.",
	...
	"snowcloak": {
		desc: "This Pokemon's Evasion is boosted by 1.11x if the weather is Hail. This Pokemon is also immune to residual Hail damage.",
		shortDesc: "If Hail is active, this Pokemon's evasion is 1.11x; immunity to Hail.",
		onImmunity: function(type, pokemon) {
			if (type === 'hail') return false;
		},
		onAccuracy: function(accuracy) {
	...
	"solidrock": {
		desc: "Super-effective attacks only deal 65% their usual damage against this Pokemon.",
		shortDesc: "This Pokemon receives 65% damage from super effective attacks.",
		onSourceModifyDamage: function(damage, source, target, move) {
			if (this.getEffectiveness(move, target) > 0) {
	...
	"terraformer": {
		desc: "On switch-in, this Pokemon sets up terrain related to its most relevant type.",
	...
	"unwavering": {
		inherit: true,
		desc: "This Pokemon gains 25% health when hit by Flying-type attacks. Pokemon is immune to Flying-type moves.",
	...
	"willpower": {
		inherit: true,
		desc: "When this Pokemon is poisoned (including Toxic), burned, paralyzed or asleep (including self-induced Rest), its Special Attack stat receives a 50% boost.",
		shortDesc: "If this Pokemon is statused, its Special Attack is 1.5x.",
		onModifySpAPriority: 5,
		onModifySpA: function(spa, pokemon) {
			if (pokemon.status) {
				return this.chainModify(1.5);
